...
class SAM_Group_Uncategorized:
    ...
    def __init__(self, ListLength, group_response = [], t=2, r=4, sam_a = .1, sam_b = .1, sam_c = .1, 
                 sam_d = .02, sam_e = 0.71, sam_f = 0.75, sam_g = 0.74, sam_j = 0.74, Kmax = 30, Lmax = 3):

        #Parameter settings based on parameter estimation in first paper:
        #sam_e = 0.71, sam_f = 0.75, sam_g = 0.74, sam_j = 0.74
        ...
        sam_j = incrementing parameter for word to other word association in group recall, specifically previous group response to current group response
    ...
    def context_recall(self):
        ...
        if self.K >= self.Kmax:
            ...
            return list([float('inf'), -1, self.K, retrieval_fails])
        ...
        while(self.K < self.Kmax):
            ...
            if (sampledTrace in self.group_response):
                ...
                if (probRecover > np.random.rand(1)[0]):#if recovery is successful, update strength of probe to the recovered memory trace
                    ...
                    return list([(time.time() - start_time), sampledTrace, self.K, retrieval_fails])
    ...
    def wordcue_recall(self, sampledTrace):
        ...
        if self.K >= self.Kmax:
            retrieval_fails.append(time.time() - start_time)
            return float('inf'), -1, self.K, retrieval_fails
        ...
        while(self.L < self.Lmax):
            ...
            if (sampledTrace in self.group_response): #if sampledTrace already said, count this as retrieval failure and start again
                ...
                if (probRecover > np.random.rand(1)[0]):#if recovery is successful, update strength of probe to the recovered memory trace
                    ...
                    return (time.time() - start_time), sampledTrace, self.K, retrieval_fails
