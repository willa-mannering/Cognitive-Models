...
def nominal_recall(group):
    ...
    for g in group:
        nominal_response.extend(g.free_recall())
...
#do group recall X amount of times
    ...
    return 'collaborative: ', np.mean(len_collab)/list_length, np.std(len_collab)/list_length, ' nominal: ', np.mean(len_nom)/list_length, np.std(len_nom)/list_length
    #return len_collab
