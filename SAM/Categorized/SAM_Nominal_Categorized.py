...
#sam_e = .65, sam_f = .54, sam_g = .94
class SAM_Nominal_Categorized:
    
    def __init__(self, ListLength, category_size, t=2, r=4, sam_a = .06, sam_b = .06, 
                 sam_c = .06, sam_d = .02, sam_e = .7, sam_f = .7, sam_g = .7, 
                 sam_h = .15, sam_i = .005, Kmax = 28, Lmax = 3):
    ...
    def free_recall(self):
        ...
        category_response = [] #list of categories represented throughout recall
        ...
        while(self.K < self.Kmax):
            ...
            if (alreadySaid[sampledTrace]):
                ...
                if (probRecover > np.random.rand(1)[0]):#if recovery is successful, update strength of probe to the recovered memory trace
                    ...
                    category_response.append(self.get_category(sampledTrace))
...
def individual_recall(num_runs, ListLength, category_size):
    ...
    for i in range(num_runs):
        ...
        sam = SAM_Nominal_Categorized(ListLength, category_size)
        
        words_recalled, cr = sam.free_recall()
        
        
        
        categories_recalled = set(cr)
        
        overall_recall.append(len(words_recalled))
        
        category_recall.append(len(categories_recalled))

        instances_recall.append(len(words_recalled)/int(ListLength/category_size))
            
    
    #return 'Overall Recall: {} Category Recall: {} Instance Recall: {}'.format(np.mean(overall_recall)/ListLength, np.mean(category_recall), round(np.mean(instances_recall),4))   
    return 'mean: {}, std: {}'.format(np.mean(overall_recall)/ListLength, np.std(overall_recall)/ListLength)       
